#!/usr/bin/env python3
"""
Train the memory system on comprehensive knowledge about the MAPI project
"""
import requests
import time
import sys
...
# Comprehensive knowledge about MAPI project
mapi_knowledge = [
    # Project Overview
    {
...
def train_memory():
    ...
    for i, knowledge in enumerate(mapi_knowledge, 1):
        try:
            ...
            r = requests.post(f"{URL}/mem/write", json=payload, timeout=10)
            r.raise_for_status()
            
            result = r.json()
            print(f"✓ [{i:3d}/{len(mapi_knowledge)}] Stored: {knowledge['text'][:70]}...")
            print(f"  ID: {result.get('id', 'N/A')}, Tags: {', '.join(knowledge.get('tags', []))}")
            success_count += 1
            time.sleep(0.1)  # Small delay to avoid overwhelming the API
            
        except requests.exceptions.ConnectionError:
            print(f"\n❌ Error: Cannot connect to API at {URL}")
